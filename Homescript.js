...
BTN.addEventListener('click', () => {
  BTN.setAttribute('disabled', true);
    tapText.style.display = 'none'; // show after click
  MASTER_TL.restart();

  // After animation, go to next page
  setTimeout(() => {
    window.location.href = "Cake.html";
  }, 24000);   // wait 9 seconds for animation to finish
});


// Enable sound by default
SOUNDS.TUNE.muted = false;
SOUNDS.MATCH.muted = false;
SOUNDS.HORN.muted = false;
SOUNDS.POP.muted = false;
SOUNDS.CHEER.muted = false;
SOUNDS.BLOW.muted = false;
SOUNDS.ON.muted = false;
